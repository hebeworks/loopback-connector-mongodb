...
// Commands that return cursors in MongoDB driver v4
// These should not have callbacks added to args
...
MongoDB.prototype.connect = function(callback) {
  var self = this;
  // Check client health, not just db existence
  if (self.client && !self.client.topology?.isDestroyed()) {
    // Client is healthy, ensure db reference exists
    if (!self.db && self.client) {
      self.db = self.client.db(self.settings.database);
    }
    process.nextTick(function() {
    ...
    // Client is unhealthy or missing - clear stale references
    if (self.client && self.client.topology?.isDestroyed()) {
      self.client = null;
      self.db = null;
    }
...
MongoDB.prototype.collection = function(model) {
  // Check client health, not just db existence
  if (!this.client || this.client.topology?.isDestroyed()) {
    throw new Error(g.f('{{MongoDB}} connection is not established'));
  }
  // Ensure db reference exists
...
MongoDB.prototype.execute = function(model, command) {
  ...
  // Check client health instead of db.topology to avoid race conditions
  // The client manages the connection pool and should only be closed on shutdown
  if (self.client && !self.client.topology?.isDestroyed()) {
    // Client is healthy, ensure db reference exists
    if (!self.db && self.client) {
      self.db = self.client.db(self.settings.database);
    ...
    // Client is unhealthy or missing - clear stale references without calling disconnect()
    // Calling disconnect() here closes the pool for ALL concurrent operations
    self.client = null;
    self.db = null;
  ...
  function doExecute() {
    ...
    self.notifyObserversAround(
      ...
      function(context, done) {
        ...
        // MongoDB driver v4: find and aggregate return cursors, not results
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  this.execute(model, 'insertOne', data, {safe: true}, function(err, result) {
    ...
    // MongoDB driver v4: insertOne returns { insertedId } not { ops: [...] }
...
MongoDB.prototype.save = function(model, data, options, callback) {
  ...
  this.execute(model, 'updateOne', {_id: oid}, {$set: data}, {upsert: true}, function(err, result) {
    ...
    var info = {};
    // MongoDB driver v4: updateOne returns { acknowledged, matchedCount, modifiedCount, upsertedId, upsertedCount }
    if (result) {
      if (result.acknowledged && result.matchedCount === 1) {
        info.isNewInstance = !!result.upsertedId;
    ...
    if (callback) {
      // Driver v4: no ops array, return the data object we just saved
      callback(err, err ? null : data, info);
    }
  });
};
...
MongoDB.prototype.destroy = function destroy(model, id, options, callback) {
  ...
  this.execute(model, 'deleteOne', {_id: id}, function(err, result) {
    ...
    // MongoDB driver v4: deleteOne returns { acknowledged, deletedCount }
    var res = result && (result.deletedCount !== undefined ?
      {count: result.deletedCount} :
      result.result ? {count: result.result.n} : undefined);
    if (callback) {
      callback(err, res);
    }
  });
...
MongoDB.prototype.destroyAll = function destroyAll(
  ...
  this.execute(model, 'deleteMany', where || {}, function(err, info) {
    ...
    // MongoDB driver v4: deleteMany returns { acknowledged, deletedCount }
    var affectedCount = info.deletedCount !== undefined ?
      info.deletedCount :
      (info.result ? info.result.n : undefined);
...
MongoDB.prototype.replaceWithOptions = function(model, id, data, options, cb) {
  ...
  this.execute(model, 'replaceOne', {_id: id}, data, options, function(
    ...
    var result;
    var cbInfo = {};
    // MongoDB driver v4: replaceOne returns { acknowledged, matchedCount, modifiedCount, upsertedId, upsertedCount }
    if ((info.matchedCount === 1) || (info.result && info.result.n == 1)) {
      result = data;
      delete result._id;
      result[idName] = id;
      // Driver v4 uses modifiedCount, v3 uses result.nModified
      if (info.modifiedCount !== undefined) {
        cbInfo.isNewInstance = info.modifiedCount === 0;
      } else if (info.result && info.result.nModified !== undefined) {
...
MongoDB.prototype.update = MongoDB.prototype.updateAll = function updateAll(
  ...
  this.execute(
    ...
    function(err, info) {
      ...
      // MongoDB driver v4: updateMany returns { acknowledged, matchedCount, modifiedCount, upsertedId, upsertedCount }
      var affectedCount = info.modifiedCount !== undefined ?
        info.modifiedCount :
        (info.result ? info.result.n : undefined);
...
MongoDB.prototype.disconnect = function(cb) {
  ...
  // Note: db.unref() removed - deprecated no-op in MongoDB driver v4.x
  if (this.client) {
    this.client.close();
  }
  if (cb) {
...
MongoDB.prototype.ping = function(cb) {
  ...
  if (self.client && !self.client.topology?.isDestroyed()) {
    ...
    // Client is unhealthy or missing - clear stale references and reconnect
    if (self.client && self.client.topology?.isDestroyed()) {
      self.client = null;
      self.db = null;
    }
